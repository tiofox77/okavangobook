<div class="py-6 px-4" x-data="{ confirmingDeletion: false, itemIdToDelete: null }">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
            <span class="inline-flex items-center">
                <i class="fas fa-utensils mr-2 text-green-600"></i>
                Gestão do Menu do Restaurante
            </span>
        </h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6 animate__animated animate__fadeIn">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <!-- Filtro de Categoria -->
            <div>
                <label for="filterCategory" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Categoria</label>
                <select id="filterCategory" wire:model.live="filterCategory" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Atributos</th>
                        ...
                        <tr wire:key="item-{{ $item->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $item->hotel->name }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">ID: #{{ $item->hotel_id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    @if($item->image)
                                        <img src="{{ asset('storage/' . $item->image) }}" class="h-12 w-12 rounded-lg object-cover mr-3 border-2 border-gray-100 dark:border-gray-700" alt="{{ $item->name }}">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        @if($item->description)
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ Str::limit($item->description, 50) }}</div>
                                        @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">
                                <div class="flex items-center font-mono bg-yellow-50 dark:bg-yellow-900/20 rounded-lg px-3 py-1.5 justify-center">
                                    ...
                                    <span class="font-bold text-yellow-700 dark:text-yellow-300">{{ number_format($item->price, 2, ',', '.') }}</span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @if($item->is_vegetarian)
                                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800" title="Vegetariano">
                                            <i class="fas fa-leaf"></i>
                                        </span>
                                    @endif
                                    @if($item->is_vegan)
                                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800" title="Vegano">
                                            <i class="fas fa-seedling"></i>
                                        </span>
                                    @endif
                                    @if($item->is_gluten_free)
                                        <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800" title="Sem Glúten">
                                            GF
                                        </span>
                                    @endif
                                    @if($item->is_spicy)
                                        <span class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-800" title="Picante">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <button wire:click="edit({{ $item->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Editar">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button 
                                        @click="confirmingDeletion = true; itemIdToDelete = {{ $item->id }}" 
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center py-6">
                                    ...
                                    @if($search || $filterHotel || $filterCategory)
                    ...
                    <form wire:submit.prevent="save">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select wire:model="hotel_id" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="name" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                            ...
                            <div>
                                ...
                                <select wire:model="category" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                            ...
                            <!-- Preço -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Preço (Kz) *</label>
                                <input type="number" step="0.01" wire:model="price" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                @error('price') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                            
                            <!-- Tempo de Preparação -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tempo de Preparação (min)</label>
                                <input type="number" wire:model="preparation_time" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="file" wire:model="imageUpload" accept="image/*" class="w-full">
                            ...
                            <div class="md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model="is_available" class="rounded">
                                    <span class="ml-2 text-sm">Disponível</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model="is_vegetarian" class="rounded">
                                    <span class="ml-2 text-sm">Vegetariano</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model="is_vegan" class="rounded">
                                    <span class="ml-2 text-sm">Vegano</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model="is_gluten_free" class="rounded">
                                    <span class="ml-2 text-sm">Sem Glúten</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" wire:model="is_spicy" class="rounded">
                                    <span class="ml-2 text-sm">Picante</span>
                                </label>
                            </div>
                        </div>
                ...
                <form wire:submit.prevent="save">
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="mb-6 flex items-center">
                            <div class="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 dark:bg-blue-900 mr-4">
                                <i class="fas fa-utensils text-blue-600 dark:text-blue-400 text-lg"></i>
                            </div>
                            <div>
                                <h3 class="text-xl leading-6 font-bold text-gray-900 dark:text-white">
                                    {{ $itemId ? 'Editar Item do Menu' : 'Adicionar Item ao Menu' }}
                                </h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Categoria *</label>
                                    <select wire:model.defer="category" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="">Selecione...</option>
                                        @foreach($categories as $value => $label)
                                            <option value="{{ $value }}">{{ $label }}</option>
                                        @endforeach
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Preço (AOA) *</label>
                                    <input type="number" step="0.01" wire:model.defer="price" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    @error('price') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tempo (min)</label>
                                    <input type="number" wire:model.defer="preparation_time" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                </div>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descrição</label>
                                <textarea wire:model.defer="description" rows="3" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500"></textarea>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Imagem</label>
                                <input type="file" wire:model="imageUpload" accept="image/*" class="w-full text-sm">
                                @if($image && !$imageUpload)
                                    <img src="{{ asset('storage/' . $image) }}" class="mt-2 h-20 w-20 object-cover rounded">
                                @endif
                                @error('imageUpload') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>
                            
                            <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Atributos</label>
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" wire:model.defer="is_vegetarian" class="rounded text-green-600 focus:ring-green-500">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Vegetariano</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" wire:model.defer="is_vegan" class="rounded text-green-600 focus:ring-green-500">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Vegano</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" wire:model.defer="is_gluten_free" class="rounded text-amber-600 focus:ring-amber-500">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Sem Glúten</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" wire:model.defer="is_spicy" class="rounded text-red-600 focus:ring-red-500">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Picante</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-4 sm:px-6 flex justify-end space-x-3 border-t border-gray-200 dark:border-gray-600">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                            <i class="fas fa-save mr-2"></i> {{ $itemId ? 'Atualizar' : 'Guardar' }}
                        </button>
                    </div>
                </form>
            </div>
